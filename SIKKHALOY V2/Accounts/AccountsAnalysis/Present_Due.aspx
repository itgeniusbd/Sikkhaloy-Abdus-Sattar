...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    <asp:FormView ID="TotalPDueFormView" runat="server" DataSourceID="IntotalPDueSQL" Width="100%">
        <ItemTemplate>
            <h3>Total Current Dues In Institution: <%# Eval("PresentDue", "{0:N0}") %> Tk</h3>
        </ItemTemplate>
    </asp:FormView>
    <asp:SqlDataSource ID="IntotalPDueSQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>"
        SelectCommand="SELECT ISNULL(SUM(CASE WHEN po.EndDate &lt; DATEADD(day,-1,CAST(GETDATE() AS date)) THEN ISNULL(po.Amount,0)+ISNULL(po.LateFee,0)-ISNULL(po.Discount,0)-ISNULL(po.PaidAmount,0)-ISNULL(po.LateFee_Discount,0) ELSE ISNULL(po.Amount,0)-ISNULL(po.Discount,0)-ISNULL(po.PaidAmount,0) END),0) AS PresentDue FROM Income_PayOrder po INNER JOIN Student s ON po.StudentID = s.StudentID WHERE po.SchoolID = @SchoolID AND po.EducationYearID = @EducationYearID AND po.Status = N'Due' AND po.Is_Active = 1 AND po.EndDate &lt; CAST(GETDATE() AS date) AND s.Status = N'Active'"
        EnableCaching="True" CacheDuration="120" OnSelecting="IntotalPDueSQL_Selecting">
        <SelectParameters>
                    ...
                    <div class="Print_Due">
                        <div class="NoPrint form-inline">
                            ...
                            <div class="form-group">
                                ...
                                <asp:SqlDataSource ID="RoleSQL" runat="server" CancelSelectOnNullParameter="False" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" SelectCommand="SELECT DISTINCT ir.Role, ir.RoleID FROM Income_PayOrder po INNER JOIN Income_Roles ir ON po.RoleID = ir.RoleID INNER JOIN StudentsClass sc ON po.StudentID = sc.StudentID AND sc.EducationYearID = @EducationYearID AND sc.ClassID = @ClassID AND sc.Class_Status IS NULL WHERE po.SchoolID = @SchoolID AND po.EducationYearID = @EducationYearID AND po.Status = N'Due' AND po.Is_Active = 1 AND po.EndDate &lt; CAST(GETDATE() AS date) ORDER BY ir.Role" OnSelecting="RoleSQL_Selecting">
                                    <SelectParameters>
                                        <asp:SessionParameter Name="SchoolID" SessionField="SchoolID" />
                                        <asp:SessionParameter Name="EducationYearID" SessionField="Edu_Year" />
                                        <asp:ControlParameter ControlID="ClassDropDownList" Name="ClassID" PropertyName="SelectedValue" />
                        ...
                        <asp:GridView ID="TotalDueGridView" runat="server" AutoGenerateColumns="False" DataSourceID="TotalDueSQL"
                            DataKeyNames="ID,Due,SMSPhoneNo,StudentID,StudentsName,RollNo" CssClass="mGrid" OnDataBound="TotalDueGridView_DataBound" AllowSorting="True">
                            <Columns>
                                <asp:TemplateField>
                                    <HeaderTemplate>
                        ...
                        <asp:SqlDataSource ID="TotalDueSQL" runat="server" CancelSelectOnNullParameter="False" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" SelectCommand="SELECT po.StudentID, s.SchoolID, s.ID, s.StudentsName, sc.RollNo, SUM(CASE WHEN po.EndDate &lt; DATEADD(day,-1,CAST(GETDATE() AS date)) THEN ISNULL(po.Amount,0)+ISNULL(po.LateFee,0)-ISNULL(po.Discount,0)-ISNULL(po.PaidAmount,0)-ISNULL(po.LateFee_Discount,0) ELSE ISNULL(po.Amount,0)-ISNULL(po.Discount,0)-ISNULL(po.PaidAmount,0) END) AS Due, s.SMSPhoneNo FROM Income_PayOrder po INNER JOIN Student s ON po.StudentID = s.StudentID INNER JOIN StudentsClass sc ON s.StudentID = sc.StudentID AND sc.EducationYearID = @EducationYearID AND sc.ClassID = @ClassID AND sc.Class_Status IS NULL WHERE po.SchoolID = @SchoolID AND po.EducationYearID = @EducationYearID AND po.Status = N'Due' AND po.Is_Active = 1 AND po.EndDate &lt; CAST(GETDATE() AS date) AND s.Status = N'Active' AND sc.SectionID LIKE @SectionID AND CAST(po.RoleID AS NVARCHAR(50)) LIKE @RoleID GROUP BY s.StudentsName, s.ID, po.StudentID, s.SMSPhoneNo, sc.RollNo, s.SchoolID ORDER BY CASE WHEN ISNUMERIC(sc.RollNo) = 1 THEN CAST(REPLACE(REPLACE(sc.RollNo, '$', ''), ',', '') AS INT) ELSE 0 END" OnSelecting="TotalDueSQL_Selecting">
                            <SelectParameters>
                                <asp:SessionParameter Name="SchoolID" SessionField="SchoolID" />
                                <asp:SessionParameter Name="EducationYearID" SessionField="Edu_Year" />
                                <asp:ControlParameter ControlID="ClassDropDownList" Name="ClassID" PropertyName="SelectedValue" />
