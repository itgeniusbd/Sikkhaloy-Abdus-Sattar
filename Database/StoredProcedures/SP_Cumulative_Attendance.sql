...
BEGIN
	...
	SET NOCOUNT ON;
--------[[[[[[[Delete]]]]]]]-------------------
DELETE FROM Attendance_Student WHERE (SchoolID = @SchoolID) AND (ClassID = @ClassID) AND (EducationYearID = @EducationYearID) AND (CumulativeNameID = @CumulativeNameID)
IF(@From_Date <> '' and @To_Date  <> '')
BEGIN 
--------[[[[[[[INSERT]]]]]]]-------------------
INSERT INTO Attendance_Student
                         (SchoolID, RegistrationID, EducationYearID,CumulativeNameID, ClassID, StudentID,  StudentClassID, WorkingDays, TotalPresent, TotalAbsent, TotalLate, TotalLeave, TotalBunk,TotalLateAbs)

SELECT       @SchoolID, @RegistrationID, @EducationYearID, @CumulativeNameID, @ClassID,  StudentsClass.StudentID, Attendance_Record.StudentClassID, COUNT(Attendance_Record.StudentClassID) AS WorkingDay, ISNULL(T_Pre.Pre, 0) AS Pre, ISNULL(T_Abs.Abs, 0) AS Abs,  ISNULL(T_Late.Late, 0) AS Late,ISNULL(T_Leave.Leave, 0) AS Leave, ISNULL(T_Bunk.Bunk, 0) AS Bunk, ISNULL(T_LateAbs.LateAbs, 0) AS LateAbs
